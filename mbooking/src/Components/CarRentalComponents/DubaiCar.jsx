...
let obj = {
  ...
  const Dubai = async () => {
    try {
      let res = await axios.get(`http://localhost:8080/dubai`,obj);
      setCar(res.data);
    } catch (err) {
      console.log("err", err);
    }
  ...
  return (
    <div>
      ...
      {/* //Recommended pick-up locations in Dubai */}
      <br />
      <Box width={"80%"} margin="auto">
        <Text textAlign={"left"} fontWeight="700" fontSize={"2xl"}>
          Recommended pick-up locations in Dubai
      ...
      {/* //Car hire destinations near Dubai */}
      <br />
      <FrequentAsk />
      <br />
      <Box width={"80%"} margin="auto">
        <Text fontSize="2xl" style={{ textAlign: "left", fontWeight: "700" }}>
          Popular car rental destinations in the Dubai
        </Text>
      </Box>
      <br />
